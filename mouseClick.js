...
canvas.addEventListener('mousedown', function() {
  if(towerAllowed(mouse.x,mouse.y)) {
    ...
    console.log('You just laid down a ' + currentTower + ' tower.');
    for(i = 0, j = towers.length; i < j; i++) {
      console.log('Tower #' + i + ' = ' + towers[i].name);
      console.log('Tower color is ' + towers[i].color);
    }
    console.log('Number of towers in array: ' + towers.length);
    money -= towerClasses[currentTower].prototype.cost;
    console.log('You just spent ' + towerClasses[currentTower].prototype.cost);
...
function drawMouse() {
  ...
  if(towerAllowed(mouse.x,mouse.y)) context.fillStyle='green';
